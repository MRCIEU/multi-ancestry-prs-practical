...
```{r}
dat <- read.csv("individual_values.csv")
str(dat)
```
...
effect_estimates <- lapply(c("EUR", "AFR", "EAS", "SAS", "AMR"), \(p) {
    ...
    mod <- summary(lm(phen ~ rs58903867, data=subset(dat, pop == p)))
...
```{r}
dat %>% group_by(pop) %>% summarise(mean_alcohol=mean(alcohol))
```
...
effect_estimates <- lapply(c(0, 1), \(p) {
    # The linear model is estimating the effect of the SNP on phen in that subgroup
    mod <- summary(lm(phen ~ rs58903867, data=subset(dat, alcohol == p)))
    # Save the effect estimate and standard error
...
```{r}
ggplot(dat, aes(x=true_score, fill=pop)) +
...
```{r}
cor(dat$eur_score[dat$pop == "EUR"], dat$phen[dat$pop == "EUR"])^2
```
